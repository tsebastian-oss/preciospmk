-- Make automotive observation ingestion idempotent within a crawl run.
-- Multiple dealer URLs can legitimately resolve to the same canonical vehicle/version.
-- Keep one observation per (product_id, crawl_run_id) and refresh it instead of failing the task.
