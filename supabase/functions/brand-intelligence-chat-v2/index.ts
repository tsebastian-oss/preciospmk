...
const cleanMsgs=(v:unknown):Msg[]=>Array.isArray(v)?v.filter((x:any)=>x&&(x.role==="user"||x.role==="assistant")&&typeof x.content==="string").map((x:any)=>({role:x.role,content:x.content.trim().slice(0,2500),brand:typeof x.brand==="string"?x.brand:null})).filter((x:Msg)=>x.content).slice(-14):[];
...
function models(mode:Mode,configured?:string|null){const validConfigured=configured&&(/^(gpt-5(?:\.[0-9]+)?(?:-[a-z0-9.-]+)?|gpt-4\.1|o3|o4-mini)$/i.test(configured))?configured:null;const base=mode==="deep"?["gpt-5.1","gpt-5","o3","gpt-4.1"]:mode==="analyst"?["gpt-5.1","gpt-5-mini","gpt-5","o3","gpt-4.1"]:["gpt-5-mini","gpt-5.1","gpt-5","gpt-4.1"];return [validConfigured,...base].filter((x,i,a):x is string=>Boolean(x)&&a.indexOf(x)===i);}
...
async function runAgent(apiKey:string,model:string,mode:Mode,messages:Msg[],base:any,execute:(name:string,args:any)=>Promise<any>){
  ...
  for(let round=0;round<4;round++){
    ...
    const calls=(raw.output??[]).filter((x:any)=>x?.type==="function_call");
  ...
  if(previous){
    ...
    const finalRaw=await callResponses(apiKey,finalBody);
    const finalText=outputText(finalRaw);
    if(finalText)return{text:finalText,trace,response:finalRaw};
    lastRaw=finalRaw;
  }
...
function deterministicFallback(brand:string,ctx:any,q:string){
  ...
  const skus=Number(s?.skus??0);
  const retailers=Number(s?.retailers??s?.chains??s?.supermarkets??0);
  const min=Number(s?.minPrice??s?.min_price??0);
  const max=Number(s?.maxPrice??s?.max_price??0);
  const avg=Number(s?.avgPrice??s?.avg_price??0);
...
Deno.serve(async(req:Request)=>{if(req.method!=="POST")return new Response("Method not allowed",{status:405});const auth=req.headers.get("authorization")??"";const url=Deno.env.get("SUPABASE_URL")!,anon=Deno.env.get("SUPABASE_ANON_KEY")!,service=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;try{const body=await req.json().catch(()=>({}));const org=typeof body?.organizationId==="string"?body.organizationId:"";const messages=cleanMsgs(body?.messages);const q=[...messages].reverse().find(m=>m.role==="user")?.content??"";if(!org||!q)return Response.json({error:"Falta la organización o la pregunta."},{status:400});const user=createClient(url,anon,{global:{headers:{Authorization:auth}}});const {data:u,error:ue}=await user.auth.getUser();if(ue||!u.user)return Response.json({error:"No autorizado"},{status:401});const svc=createClient(url,service);const {data:cfg}=await svc.rpc("get_ai_runtime_config_service");const config=(cfg??{}) as Runtime;const filters=body?.filters??{};const previousBrand=[...messages].slice(0,-1).reverse().find(m=>m.role==="assistant"&&m.brand)?.brand??null;const resolution=await resolveBrand(user,org,q,filters,previousBrand);if(!resolution.brand){if(smallTalk(q))return Response.json({answer:"Hola. Puedes preguntarme por precios, promociones, disponibilidad, surtido, tendencias o comparar marcas y cadenas. También puedo hacer análisis más profundos y darte recomendaciones basadas en los datos monitoreados.",brand:null,ai:false,analysisMode:"fast"});const suggestions=resolution.candidates.slice(0,3).map((c:any)=>c.brand);return Response.json({answer:suggestions.length?`No estoy seguro de qué marca quieres analizar. ¿Te refieres a ${suggestions.join(", ")}?`:"¿Qué marca quieres que analice?",brand:null,candidates:resolution.candidates,ai:false});}const {data:ctx,error:ce}=await currentContext(user,org,resolution.brand,q,filters);if(ce||!ctx?.found)return Response.json({answer:"Identifiqué la marca, pero no encontré observaciones vigentes con este alcance. Puedo ampliar los filtros o revisar otra presentación.",brand:resolution.brand,ai:false,warning:"no_current_observations",resolution});const mode=modeFor(q);const base={brand:resolution.brand,current:ctx.current,trend:ctx.trend,quality:ctx.quality,scope:ctx.scope,segment:ctx.segment??{...segment(q),variant:variant(q).label},resolution:{method:resolution.method}};if(!config.enabled||!config.api_key)return Response.json({answer:deterministicFallback(resolution.brand,ctx,q),brand:resolution.brand,data:ctx,ai:false,analysisMode:mode});const execute=async(name:string,args:any)=>{if(name==="get_price_position")return await pricePosition(user,org,resolution.brand,String(args.question||q),filters);if(name==="get_trend")return await trend(user,org,resolution.brand,filters,Number(args.days)||Number(filters.period)||30);if(name==="get_price_matches")return await matches(user,org,resolution.brand,filters,Number(args.limit)||15);if(name==="compare_brand")return await compareBrand(user,org,String(args.brand||""),filters);if(name==="get_market_pulse"){const {data,error}=await user.rpc("enterprise_weighted_price_pulse",{p_organization_id:org});return error?{error:error.message}:data;}return{error:"unknown_tool"};};let last="";for(const model of models(mode,config.model)){try{const result=await runAgent(config.api_key,model,mode,messages,base,execute);if(result.text)return Response.json({answer:result.text,brand:resolution.brand,model,ai:true,assistant:"MGP Intelligence",responseStyle:"agentic",analysisMode:mode,reasoningEffort:supportsReasoning(model)?effort(mode):"none",toolsUsed:result.trace,data:ctx,resolution});last="respuesta vacía";}catch(e:any){last=e?.message||String(e);const status=Number(e?.status);if(![400,403,404,408,409,429,500,502,503,504].includes(status))break;}}return Response.json({answer:deterministicFallback(resolution.brand,ctx,q),brand:resolution.brand,data:ctx,ai:false,warning:"advanced_model_unavailable",analysisMode:mode,responseStyle:"deterministic_fallback"});}catch(e){console.error(e);return Response.json({error:"No pude completar el análisis avanzado."},{status:500});}});