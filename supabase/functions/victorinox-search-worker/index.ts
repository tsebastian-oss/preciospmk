...
async function run(mode:"official"|"marketplace"){const b=await supabase.from("brands_vertical_brands").select("id").eq("slug",BRAND_SLUG).single();if(b.error)throw b.error;const sourceId=await getSource(b.data.id,mode);const cfg=await runtime();if(!cfg.enabled||!cfg.api_key)throw new Error("ai_runtime_unavailable");const m=await model(cfg.api_key);const rows=await search(cfg.api_key,m,mode);let inserted=0,priced=0;for(const row of rows){if(await persist(b.data.id,sourceId,mode,row))inserted++;if(row.current_price)priced++;}await supabase.from("brands_vertical_sources").update({last_crawled_at:new Date().toISOString(),last_status:`ok:${rows.length}:inserted:${inserted}:priced:${priced}`,last_error:null}).eq("id",sourceId);return{mode,model:m,found:rows.length,inserted,priced};}

Deno.serve(async(req:Request)=>{if(req.method!=="POST")return Response.json({error:"method_not_allowed"},{status:405});const supplied=req.headers.get("x-victorinox-worker-token");const cfg=await supabase.from("qsr_worker_config").select("token").eq("id",1).single();if(!supplied||!cfg.data?.token||supplied!==cfg.data.token)return Response.json({error:"unauthorized"},{status:401});const body=await req.json().catch(()=>({}));const mode=body?.mode==="marketplace"?"marketplace":"official";try{return Response.json({ok:true,result:await run(mode)});}catch(e){return Response.json({ok:false,error:e instanceof Error?e.message:String(e)},{status:500});}});
