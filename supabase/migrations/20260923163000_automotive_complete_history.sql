-- Preserve price context at capture time. Older records retain only the prices actually stored.
alter table public.price_observations add column if not exists automotive_snapshot jsonb;
