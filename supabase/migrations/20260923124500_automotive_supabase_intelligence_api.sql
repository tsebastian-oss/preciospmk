create or replace function public.automotive_options_rows(
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(x) order by x.brand,x.model,x.dealer),'[]'::jsonb)
  into v_rows
  from (
    select
      ...
      count(distinct p.id)::int as versions
    ...
    group by p.brand,model,p.supermarket
    order by p.brand,model,p.supermarket
    limit 12000
  ) x;
...
revoke all on function public.automotive_options_rows(uuid) from public,anon;
grant execute on function public.automotive_options_rows(uuid) to authenticated;
