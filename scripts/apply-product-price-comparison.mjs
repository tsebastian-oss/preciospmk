...
const marker = "/* product-price-comparison-v1 */";
...
if (!source.includes("mode=brands")) {
  ...
  const productEffects = `${refreshEffect}
  ...
  source = source.replace(refreshEffect, productEffects);
}

const filtersRegex = /    <section className=\{styles\.filters\}>[\s\S]*?    <\/section>\n\n    \{error &&/;
if (filtersRegex.test(source)) {
  const replacement = `    <section className={styles.productTrendFilters}>
    ...
    {error &&`;
  source = source.replace(filtersRegex, replacement);
}
...
if (source.includes('value={retailer}') || source.includes('value={category}') || source.includes('value={brand}')) {
  throw new Error("Legacy overview filters are still active");
}
...
console.log("ClickHouse product price comparison applied");
