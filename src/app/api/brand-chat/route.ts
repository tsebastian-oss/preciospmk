...
async function createConversation(token: string, organizationId: string, question: string) {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/brand_ai_conversations?select=id,title`, {
    ...
    body: JSON.stringify({ organization_id: organizationId, title: conversationTitle(question) }),
    cache: "no-store",
    signal: AbortSignal.timeout(8_000),
  });
  const data = await readJson(response);
  if (!response.ok || !Array.isArray(data) || !data[0]?.id) {
    throw new Error(data?.message || "No fue posible crear la conversación.");
  }
...
async function saveMessage(
  ...
  if (!response.ok) {
    const data = await readJson(response);
    throw new Error(data?.message || "No fue posible guardar el mensaje.");
  }
}
