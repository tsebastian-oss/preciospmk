...
export default function B2BDecisionLab({ zones, selectedMonth, months, onMonthChange }: Props) {
  ...
  const opportunityRows = useMemo(() => zones.map(({ zone, rows }) => {
    ...
    return {
      ...
      premium,
      opportunityScore,
      action: actionForPremium(premium),
    };
  }), [zones]);
  ...
  const recommendedLow =
    leaderPrice > 0 && currentPrice > 0
      ? Math.round(Math.max(marginFloorPrice, leaderPrice * 1.10))
      : 0;
  const recommendedHigh =
    leaderPrice > 0 && currentPrice > 0
      ? Math.round(Math.max(recommendedLow, Math.min(currentPrice * 0.92, leaderPrice * 1.60)))
      : 0;
  const recommendedMid =
    recommendedLow > 0 && recommendedHigh > 0
      ? Math.round((recommendedLow + recommendedHigh) / 2)
      : 0;
  const discountVsCurrent =
    currentPrice > 0 && recommendedMid > 0
      ? (recommendedMid / currentPrice - 1) * 100
      : 0;
  ...
  return <div className={styles.shell}>
    ...
    <section className={styles.block}>
      <div className={styles.blockHead}>
        <div>
          <span>01 · MAPA DE OPORTUNIDADES</span>
          <h3>¿Dónde debería actuar Chilexpress primero?</h3>
        </div>
        <small>Score basado en brecha de precio y confianza del benchmark.</small>
      </div>

      <div className={styles.opportunityGrid}>
        {opportunityRows.map((item) => <button
          type="button"
          key={item.zone}
          className={selectedZone === item.zone ? styles.opportunityActive : styles.opportunityCard}
        ...
        >
          ...
          <div className={styles.opportunityStats}>
            <div>
              ...
              <strong>{item.chilexpress ? money.format(item.chilexpress.price) : "—"}</strong>
            </div>
            <div>
              <span>Líder</span>
              <strong>{item.leader ? `${item.leader.label} · ${money.format(item.leader.price)}` : "—"}</strong>
            </div>
            <div>
              <span>Premium</span>
              <strong>{item.premium == null ? "—" : `+${pct.format(item.premium)}%`}</strong>
            </div>
          </div>
    ...
    <section className={styles.block}>
      <div className={styles.blockHead}>
        <div>
          <span>02 · IMPACTO COMERCIAL</span>
          <h3>¿Qué pasa si cambiamos precio y volumen?</h3>
        </div>
      ...
      <div className={styles.impactGrid}>
        <div className={styles.assumptions}>
          <label>
            Zona
            <select value={selectedZone} onChange={(event) => setSelectedZone(event.target.value as MacroZone)}>
              {opportunityRows.map((item) => <option key={item.zone}>{item.zone}</option>)}
            </select>
          </label>

          <label>
            Volumen mensual actual
            <input type="number" min={0} step={100} value={monthlyVolume} onChange={(event) => setMonthlyVolume(Math.max(0, Number(event.target.value) || 0))}/>
          </label>

          <label>
            Cambio de precio
            <div className={styles.rangeHeader}><b>{priceChange > 0 ? "+" : ""}{priceChange}%</b><span>vs precio actual</span></div>
            <input className={styles.range} type="range" min={-30} max={20} step={1} value={priceChange} onChange={(event) => setPriceChange(Number(event.target.value))}/>
          </label>

          <label>
            Cambio esperado de volumen
            <div className={styles.rangeHeader}><b>{volumeChange > 0 ? "+" : ""}{volumeChange}%</b><span>supuesto demo</span></div>
            <input className={styles.range} type="range" min={-20} max={50} step={1} value={volumeChange} onChange={(event) => setVolumeChange(Number(event.target.value))}/>
          </label>

          <label>
            Costo unitario supuesto
            <div className={styles.rangeHeader}><b>{costShare}%</b><span>del precio actual</span></div>
            <input className={styles.range} type="range" min={35} max={85} step={1} value={costShare} onChange={(event) => setCostShare(Number(event.target.value))}/>
          </label>
        </div>
        ...
        <div className={styles.impactResults}>
          <article>
            ...
            <strong>{currentPrice ? money.format(currentPrice) : "—"}</strong>
            <small>{selectedZone}</small>
          </article>
          <article>
            <span>Nuevo precio</span>
            <strong>{newPrice ? money.format(newPrice) : "—"}</strong>
            <small>{priceChange > 0 ? "+" : ""}{priceChange}%</small>
          </article>
          <article>
            <span>Ingresos mensuales</span>
            <strong>{money.format(newRevenue)}</strong>
            <small>{newRevenue >= currentRevenue ? "+" : ""}{pct.format(currentRevenue ? (newRevenue / currentRevenue - 1) * 100 : 0)}% vs actual</small>
          </article>
          <article>
            <span>Contribución estimada</span>
            <strong>{money.format(newContribution)}</strong>
            <small>{newContribution >= currentContribution ? "+" : ""}{pct.format(currentContribution ? (newContribution / currentContribution - 1) * 100 : 0)}% vs actual</small>
          </article>
          <article>
            <span>Margen estimado</span>
            <strong>{pct.format(newMargin)}%</strong>
            <small>actual: {pct.format(currentMargin)}%</small>
          </article>
          <article>
            <span>Volumen modelado</span>
            <strong>{Math.round(newVolume).toLocaleString("es-CL")}</strong>
            <small>envíos / mes</small>
          </article>
        </div>
      </div>
    ...
    <section className={styles.block}>
      <div className={styles.blockHead}>
        <div>
          <span>03 · PRECIO RECOMENDADO DE TEST</span>
          <h3>¿Qué rango conviene poner a prueba?</h3>
        </div>
      ...
      <div className={styles.recommendGrid}>
        <div className={styles.recommendControls}>
          ...
          <label>
            ...
            <div className={styles.rangeHeader}><b>{targetMargin}%</b><span>sobre precio de test</span></div>
            <input className={styles.range} type="range" min={10} max={45} step={1} value={targetMargin} onChange={(event) => setTargetMargin(Number(event.target.value))}/>
          </label>
