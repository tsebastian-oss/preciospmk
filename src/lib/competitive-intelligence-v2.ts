...
function relationship(params:{score:number;sameBrand:boolean;lexical:number;category:number;packageScore:number}):Relationship|null{
  const {score,sameBrand,lexical,category,packageScore}=params;
  if(sameBrand&&lexical>=.45&&packageScore>=.82&&score>=.62)return "equivalent";
  if(packageScore>=.68&&((category>=.55&&lexical>=.27)||lexical>=.47)&&score>=.5)return "direct_competitor";
...
export function scoreCompetitor(target:ProductRecord,candidate:ProductRecord):CompetitorMatch|null{
  ...
  const relation=relationship({score:raw,sameBrand,lexical,category,packageScore});if(!relation)return null;
